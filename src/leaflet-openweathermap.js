...
L.OWM.Current = L.Layer.extend({
  ...
  initialize: function (options) {
    ...
    this._map = null;
    this._urlTemplate =
      'box/{type}?{appId}cnt=300&format=json&units=metric&bbox={minlon},{minlat},{maxlon},{maxlat},10';
    this._directions = [
      'N',
      'NNE',
      'NE',
  ...
  _createMarker: function (station) {
    ...
    const marker = L.marker([station.coord.Lat, station.coord.Lon], {
      icon: icon,
    });
    return marker;
  },
...
L.OWM.CurrentCache = L.Class.extend({
  ...
  get: function (minLon, minLat, maxLon, maxLat) {
    ...
    const clippedStations = new Array();
    let cnt = 0;
